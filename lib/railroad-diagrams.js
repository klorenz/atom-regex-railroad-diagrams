...
var options = {
	...
	function NonTerminal(text, title) {
		if(!(this instanceof NonTerminal)) return new NonTerminal(text, title);
		FakeSVG.call(this, 'g');
		this.text = text;
		if (title) {
			this.title = title;
		}
		this.width = text.length * 8 + 12;
		this.up = 11;
		this.down = 11;
	}
	...
	function Comment(text, attrs) {
		...
		this.paddingH = 2
		this.paddingV = 5
		this.width = text.length * ((this.length < 5) ? 7 : 6) + this.paddingH*2;
		this.attrs = attrs || {};
		this.up = 11;
		this.down = 11;
	}
	...
	Comment.prototype.format = function(x, y, width) {
		...
		attrs.x = x+this.width/2;
		attrs.y = y+this.paddingV;
		if (!attrs.class) {
			attrs.class = 'comment';
		} else {
			attrs.class += " comment";
